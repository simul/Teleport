...
public:
	...
	virtual ovrSurfaceDef 	TextSurface( const char * text,
			...
			VerticalJustification vjust, fontParms_t const * fp = nullptr ) const = 0;

    virtual Vector2f        GetScaleFactor() const = 0;
...
public:
	...
	// Draw functions returns the amount to modify position by for the next draw call if you want to render more lines and have them spaced normally. Will be along the vector up.
