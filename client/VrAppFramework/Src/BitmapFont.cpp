...
static char const * SDFFontFragmentShaderSrc =
...
public:
	ovrFontWeight( float const alphaCenterOffset = 0.0f, float const colorCenterOffset = 0.0f )
		: AlphaCenterOffset( alphaCenterOffset )
		, ColorCenterOffset( colorCenterOffset )
	{
	}
...
public:
	...
	virtual ovrSurfaceDef 	TextSurface( const char * text,
			...
			VerticalJustification vjust, fontParms_t const * fontParms = nullptr ) const;
...
struct ovrFormat
{
	ovrFormat( uint32_t const color )
	: Color( color )
	, Weight( 0xffffffff )
	, LastWeight( 0xffffffff )
	{
	}
...
static void UpdateFormat( FontInfoType const & fontInfo, fontParms_t const & fontParms,
		char const ** buffer, ovrFormat & format, uint8_t vertexParms[4] )
{
...
//==============================
// DrawText3D
VertexBlockType DrawTextToVertexBlock( BitmapFont const & font, fontParms_t const & fontParms,
		Vector3f const & pos, Vector3f const & normal, Vector3f const & up,
		float scale, Vector4f const & color, char const * text, Vector3f * toNextLine = nullptr )
{
	if ( toNextLine )
	{
...
///			width, height, numLines, AsLocal( font ).GetFontInfo().FontHeight );
	...
	for ( ; charCode != '\0'; i++, charCode = UTF8Util::DecodeNextChar( &p ) )
	{
        ...
        v[i * 4 + 0].s = s0;
        v[i * 4 + 0].t = t1;
        *(UInt32*)(&v[i * 4 + 0].rgba[0]) = format.Color;
		*(UInt32*)(&v[i * 4 + 0].fontParms[0]) = *(UInt32*)(&vertexParms[0]);
	    // upper left
        v[i * 4 + 1].xyz = curPos + ( r * bearingX ) + ( u * bearingY );
        v[i * 4 + 1].s = s0;
        v[i * 4 + 1].t = t0;
        *(UInt32*)(&v[i * 4 + 1].rgba[0]) = format.Color;
		*(UInt32*)(&v[i * 4 + 1].fontParms[0]) = *(UInt32*)(&vertexParms[0]);
        // upper right
        v[i * 4 + 2].xyz = curPos + ( r * rw ) + ( u * bearingY );
        v[i * 4 + 2].s = s1;
        v[i * 4 + 2].t = t0;
        *(UInt32*)(&v[i * 4 + 2].rgba[0]) = format.Color;
		*(UInt32*)(&v[i * 4 + 2].fontParms[0]) = *(UInt32*)(&vertexParms[0]);
        // lower right
        v[i * 4 + 3].xyz = curPos + ( r * rw ) - ( u * rh );
        v[i * 4 + 3].s = s1;
        v[i * 4 + 3].t = t1;
        *(UInt32*)(&v[i * 4 + 3].rgba[0]) = format.Color;
		*(UInt32*)(&v[i * 4 + 3].fontParms[0]) = *(UInt32*)(&vertexParms[0]);
...
ovrSurfaceDef BitmapFontLocal::TextSurface( const char * text,
		float scale, const Vector4f & color, HorizontalJustification hjust,
		VerticalJustification vjust, fontParms_t const * fontParms ) const
{
	fontParms_t	fp;
...
public:
	...
	virtual Vector3f	DrawText3D( BitmapFont const & font, const fontParms_t & flags,
			        			const Vector3f & pos, Vector3f const & normal, Vector3f const & up,
								float const scale, Vector4f const & color, char const * text );
	virtual Vector3f	DrawText3Df( BitmapFont const & font, const fontParms_t & flags,
								const Vector3f & pos, Vector3f const & normal, Vector3f const & up,
								float const scale, Vector4f const & color, char const * text, ... );

	virtual Vector3f	DrawTextBillboarded3D( BitmapFont const & font, fontParms_t const & flags,
			        			Vector3f const & pos, float const scale, Vector4f const & color,
								char const * text );
	virtual Vector3f	DrawTextBillboarded3Df( BitmapFont const & font, fontParms_t const & flags,
			        			Vector3f const & pos, float const scale, Vector4f const & color,
								char const * fmt, ... );
...
bool FontInfoType::LoadFromBuffer( void const * buffer, size_t const bufferSize )
{
	...
	// JSON doesn't have ints so cast from float to an int
	...
	while ( *cur != '\0' )
	{
		...
		if ( lineWidth >= widthMeters )
		{
			...
			else
			{
				...
				if ( overwrite )
				{
					// we overwrote with a single byte, but we could have overwritten a multi-byte unicode character
					// so we may need to shift the entire destination string down by (sizeof overwritten char) - 1 bytes.
					// decode the character at the bestBreak point to determine how many bytes we need to skip
...
Vector3f BitmapFontSurfaceLocal::DrawText3D( BitmapFont const & font, fontParms_t const & parms,
		Vector3f const & pos, Vector3f const & normal, Vector3f const & up,
...
Vector3f BitmapFontSurfaceLocal::DrawText3Df( BitmapFont const & font, fontParms_t const & parms,
		Vector3f const & pos, Vector3f const & normal, Vector3f const & up,
...
Vector3f BitmapFontSurfaceLocal::DrawTextBillboarded3D( BitmapFont const & font, fontParms_t const & parms,
		Vector3f const & pos, float const scale, Vector4f const & color, char const * text )
{
	...
	return DrawText3D( font, billboardParms, pos, Vector3f( 1.0f, 0.0f, 0.0f ), Vector3f( 0.0f, -1.0f, 0.0f ),
			scale, color, text );
}

//==============================
// BitmapFontSurfaceLocal::DrawTextBillboarded3Df
Vector3f BitmapFontSurfaceLocal::DrawTextBillboarded3Df( BitmapFont const & font, fontParms_t const & parms,
