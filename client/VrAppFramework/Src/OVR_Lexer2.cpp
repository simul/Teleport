...
ovrLexer::ovrLexer( std::vector< uint8_t > const & source, char const * punctuation )
	: ovrLexer( (const char*)( static_cast< uint8_t const* >( source.data() ) ), source.size(), punctuation )
{
}
...
void ovrLexer::EmitCodePoint( uint32_t const ch, char * buffer, ptrdiff_t & bufferOfs, size_t const bufferSize, 
