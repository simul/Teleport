...
public:
	...
	char const *	GetCur() const { return Cur; }
	void			Reset() { Cur = Source; Prev = NULL; PrevPrev = NULL; }
...
{
	int port = 0;
	return ParseUri( uri, outScheme, outSchemeSize, NULL, 0, NULL, 0, NULL, 0, port, NULL, 0, NULL, 0, NULL, 0 );
}

//==============================
// EncodeCharToBuffer
// only returns false if the buffer overflows -- NULL out pointers are just skipped
static bool EncodeCharToBuffer( uint32_t const ch, char * out, size_t const outSize, 
		ptrdiff_t & outOffset )
{
	if ( out == NULL )
	{
...
//==============================
// ovrUri::ParseUri
bool ovrUri::ParseUri( char const * uri, 
		...
		char * outQuery, size_t const outQuerySize,
		char * outFragment, size_t const outFragmentSize )
{
...
#if defined( TOLERATE_MISSING_SCHEME )	// tolerant to missing scheme, but not clear if that's desirable
	...
	if ( decoder.PeekNext() == '/' && decoder.PeekNextNext() == '/' )
	{
		...
		for ( ; ; )
		{
			...
			if ( ch == '\0' || ch  == '/' )
			{
				// we reached the end of the host section or the uri without finding a username / password
				// reset to the start of the host and continue trying to parse a host
			...
			if ( !parsedUsername )
			{
				if ( !usernameOverflowed && !EncodeCharToBuffer( ch, outUsername, outUsernameSize, usernameOffset ) )
				{
					// don't consider this an error until we're sure that we're parsing a username and not a host
			...
			{
				if ( !passwordOverflowed && !EncodeCharToBuffer( ch, outPassword, outPasswordSize, passwordOffset ) )
				{
					// don't consider this an error until we're sure that we're parsing a password and not a host
		...
		// There is a host so parse the host. The host is everything up to the next forward slash ( or colon if there is a port )
		...
		for ( ; ; )
		{
			...
			// check for the end of the host or the end of the string -- we may have just parsed the port at this point
			if ( ch == '/' || ch == '\0' )
			{
				// end of the host
				if ( outHost != NULL )
				...
				if ( ch == '\0' )
				{
					...
					// It's not enough information by itself to find a resource, but we do not know if
					// the client is going to compose a URI in parts outside of this.
					return true;
#else
					if ( hostOffset < 1 )
					{
...
bool ovrUri::IsValidUri( char const * uri )
{
	...
	bool const valid = ovrUri::ParseUri( uri, scheme, sizeof( scheme ), 
...
static void ReportTest( char const * testName, char const * uri, bool const isValid, bool const success,
		char const * scheme, char const * username, char const * password, char const * host,
		int const port, char const * path, char const * query, char const * fragment )
{
...
static void Test( char const * testName, char const * uri, bool const isValid )
{
	...
	bool const success = ovrUri::ParseUri( uri, scheme, sizeof( scheme ), 
			username, sizeof( username ), password, sizeof( password ),
			host, sizeof( host ), port, 
			path, sizeof( path ), 
			query, sizeof( query ), 
			fragment, sizeof( fragment ) );
	ReportTest( testName, uri, isValid, success, scheme, username, password,
