...
public:
	...
	bool							minMaxSet;
	// Minimum and Maximum values for each element in accessor data if the component type is a byte or int.
	int								intMin[MAX_MODEL_ACCESSOR_COMPONENT_SIZE];
	int								intMax[MAX_MODEL_ACCESSOR_COMPONENT_SIZE];
	// Minimum and Maximum values for each element in accessor data if the component type is a float.
...
typedef enum
{
	MODEL_CAMERA_TYPE_PERSPECTIVE,
	MODEL_CAMERA_TYPE_ORTHOGRAPHIC
} ModelCameraType;
...
{
	ModelCamera()
		: type( MODEL_CAMERA_TYPE_PERSPECTIVE )
	{
	}
...
public:
	...
	Matrix4f						GetLocalTransform() const { return localTransform; }
	Matrix4f						GetGlobalTransform() const { return globalTransform; }
...
{
public:
	ModelAnimationTimeLineState() : frame( 0 )
		, fraction( 0.0f )
		, timeline( nullptr )
	{
	}
...
{
public:
	ModelSubSceneState() : visible( false )
		, subScene( nullptr )
	{
	}
...
{
public:
	ModelState() : DontRenderForClientUid( 0 )
		, mf( nullptr )
	{
	...
	Matrix4f						GetMatrix() const { return modelMatrix;  } 
