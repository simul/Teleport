		...
		ovrDrawSurface & outSurf ) const
{
	...
	nextSlot = SetUniformValues( gc, nextSlot, gc.Program.uUVOffset, offsetUVs );
...
bool VRMenuObject::HitTest_r( OvrGuiSys const & guiSys, Posef const & parentPose,
...
menuHandle_t VRMenuObject::HitTest( OvrGuiSys const & guiSys, Posef const & worldPose,
	...
	do 
	{
		if ( token[0] == '#' )
		{
			...
			if ( OVR_strcmp( token, "pragma" ) == 0 )
			{
				...
				if ( OVR_strcmp( token, "overload_float_default_value" ) == 0 )
				{
