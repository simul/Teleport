...
public:
	...
	void				OneTimeInit( OvrGuiSys & guiSys, VRMenuObject * self, const Vector4f & color );
	void				SetProgressBarBaseWidth( int width ) { ProgressBarBaseWidth = static_cast< float >( width ); }
	void				SetProgressBarBaseHeight( int height ) { ProgressBarBaseHeight = static_cast< float >( height ); }
	void				SetProgressBarThumbWidth( int width ) { ProgressBarThumbWidth = static_cast< float >( width ); }
	void				SetProgressBarThumbHeight( int height ) { ProgressBarThumbHeight = static_cast< float >( height ); }
...
private:
	...
	eMsgStatus			OnInit( OvrGuiSys & guiSys, ovrFrameInput const & vrFrame,
								VRMenuObject * self, VRMenuEvent const & event );

	eMsgStatus			OnFrameUpdate( OvrGuiSys & guiSys, ovrFrameInput const & vrFrame,
								VRMenuObject * self, VRMenuEvent const & event );
