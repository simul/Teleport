...
void Fader::Update( float const fadeRate, double const deltaSeconds )
{
    ...
    {
        float const fadeDelta = static_cast<float>( fadeRate * deltaSeconds ) * ( FadeState == FADE_IN ? 1.0f : -1.0f );
        FadeAlpha += fadeDelta;
...
bool Fader::IsFadingInOrFadedIn( eFadeState const state ) const
{
	switch( FadeState )
	{
		...
		default:
			OVR_ASSERT( false );	// this should never be called with state FADE_PAUSE
			return false;
	}
}
