...
void OvrScrollManager::TouchUp()
{
	...
	else
	{
		...
		float touchVelocity = totalTime > MATH_FLOAT_SMALLEST_NON_DENORMAL ? totalDistance / totalTime : 0.0f;
...
float OvrScrollManager::GetModifiedVelocity( const float inVelocity )
{
	...
	while( true )
	{
		...
		// This is inverse to Velocity_new = Velocity_old - Velocity_old * ScrollBehavior.FrictionPerSec * 0.016f (used in OvrScrollManager::Frame function)
