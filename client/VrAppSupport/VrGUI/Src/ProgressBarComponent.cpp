...
void OvrProgressBarComponent::SetProgressFrac( OvrVRMenuMgr & menuMgr, VRMenuObject * self, const float frac )
{
	...
	if ( frac >= 0.0f && frac <= 1.0f )
	{
		...
		if ( thumb != NULL )
		{
			thumb->SetSurfaceDims( 0, Vector2f( ProgressBarCurrentThumbLength, ProgressBarThumbHeight ) );
...
void OvrProgressBarComponent::OneTimeInit( OvrGuiSys & guiSys, VRMenuObject * self, const Vector4f & color )
{
	// Set alpha on the base - move this to somewhere more explicit if needed
	VRMenuObject * base = guiSys.GetVRMenuMgr().ToObject( self->ChildHandleForId( guiSys.GetVRMenuMgr(), ProgressBarBaseId ) );
	if ( base != NULL )
	{
	...
	// start off hidden
	SetProgressFrac( guiSys.GetVRMenuMgr(), self, 0.0f ); // this forces the generation of the thumb at the right position.
...
eMsgStatus OvrProgressBarComponent::OnEvent_Impl( OvrGuiSys & guiSys, ovrFrameInput const & vrFrame,
...
eMsgStatus OvrProgressBarComponent::OnFrameUpdate( OvrGuiSys & guiSys, ovrFrameInput const & vrFrame,
		VRMenuObject * self, VRMenuEvent const & event )
{
