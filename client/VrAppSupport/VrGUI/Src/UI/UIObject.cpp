...
UIObject::~UIObject()
{
	...
	// already been freed.
	for ( UPInt i = Children.GetSize() - 1; i >= 0; i-- )
	{
...
void UIObject::SetMenuObject( UIMenu * menu, VRMenuObject * vrMenuObject, UIObject * vrMenuObjectParent )
{
	OVR_ASSERT( menu );
	OVR_ASSERT( vrMenuObject );
	OVR_ASSERT( Parent == NULL );
	OVR_ASSERT( vrMenuObjectParent == NULL
...
Posef UIObject::GetWorldPose() const
{
	...
	Posef curModelPose;
	curModelPose.Translation = parentModelPose.Translation + ( parentModelPose.Rotation * parentScale.EntrywiseMultiply( localPose.Translation ) );
...
Vector3f UIObject::GetWorldPosition() const
{
	...
	return parentModelPose.Translation + ( parentModelPose.Rotation * parentScale.EntrywiseMultiply( localPose.Translation ) );
}
...
void UIObject::AlignTo( const RectPosition myPosition, const UIObject *other, const RectPosition otherPosition, const float zOffset )
{
	...
	const Vector3f alignPosWorld = otherWorldPose.Rotation.Rotate( alignPos * DEFAULT_TEXEL_SCALE * otherScale ) + otherWorldPose.Translation;

	// convert align position from world space to parent space
	Vector3f alignPosLocal = parentWorldPose.Rotation.Inverted().Rotate( alignPosWorld - parentWorldPose.Translation );
	...
	myLocalPose.Translation = alignPosLocal - ( myAlignPos * DEFAULT_TEXEL_SCALE * GetLocalScale().x );
...
void UIObject::AlignToMargin( const RectPosition myPosition, const UIObject *other, const RectPosition otherPosition, const float zOffset )
{
	const Posef otherWorldPose = other->GetWorldPose();
	const Vector3f otherScale = other->GetWorldScale();
...
void UIObject::SetImage( const int surfaceIndex, const eSurfaceTextureType textureType, char const * imageName )
{
	VRMenuObject * object = GetMenuObject();
	OVR_ASSERT( object );
	if ( object == NULL )
...
void UIObject::SetImage( const int surfaceIndex, const eSurfaceTextureType textureType, const GLuint image, const short width, const short height )
{
	VRMenuObject * object = GetMenuObject();
	OVR_ASSERT( object );
	if ( object == NULL )
...
void UIObject::SetImage( const int surfaceIndex, const eSurfaceTextureType textureType, const UITexture &image )
{
	VRMenuObject * object = GetMenuObject();
	OVR_ASSERT( object );
	if ( object == NULL )
...
void UIObject::SetMultiTextureImage( const int surfaceIndex, const eSurfaceTextureType textureType, const UITexture &image1, const UITexture &image2 )
{
	VRMenuObject * object = GetMenuObject();
	OVR_ASSERT( object );
	if ( object == NULL )
	...
	VRMenuSurfaceParms parms( "",
		image1.Texture, image1.Width, image1.Height, textureType,
		image2.Texture, image2.Width, image2.Height, textureType,
		0, 0, 0, SURFACE_TEXTURE_MAX );
...
void UIObject::SetImage( const int surfaceIndex, const eSurfaceTextureType textureType, const UITexture &image, const float dimsX, const float dimsY )
{
	VRMenuObject * object = GetMenuObject();
	OVR_ASSERT( object );
	if ( object == NULL )
...
void UIObject::SetImage( const int surfaceIndex, const eSurfaceTextureType textureType, const UITexture &image, const float dimsX, const float dimsY, const UIRectf &border )
{
	VRMenuObject * object = GetMenuObject();
	OVR_ASSERT( object );
	if ( object == NULL )
...
void UIObject::SetImage( const int surfaceIndex, const eSurfaceTextureType textureType, const UITexture &image, const UIRectf &border )
{
	VRMenuObject * object = GetMenuObject();
	OVR_ASSERT( object );
	if ( object == NULL )
