...
namespace OVR {
	...
	void UISlider::AddToMenu( UIMenu *menu, UIObject *parent /*= NULL */, float startingFrac /*= 0.5f*/ )
	{
		Menu = menu;
		Parent = parent;
		...
		OvrSliderComponent::imageInfo_t sliderTrackFullImage 	= { "res/raw/slider_track_full_%s.png", 9, 145 };
		...
		OvrSliderComponent::GetVerticalSliderParms( *menu->GetVRMenu(),
			parent->GetId(), VolumeSliderId, 
			iconImage, sliderBaseImage, sliderTrackImage,
			sliderTrackFullImage, sliderScrubberImage, sliderBubbleImage,
			VoumeScrubberId, VolumeBubbleId, VolumeFillId,
			startingFrac, 0.0f, 10.f, 0.000625f, sliderParms );
