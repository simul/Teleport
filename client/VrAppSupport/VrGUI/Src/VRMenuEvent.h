...
{
	VRMENU_EVENT_FOCUS_GAINED,		// TargetHandle is the handle of the object gaining focus, other handle is previous focus
	...
	VRMENU_EVENT_SWIPE_COMPLETE,	// sent when a swipe operation has been completed and touch up has triggered
...
public:
	...
	VRMenuEvent()
		: EventType( VRMENU_EVENT_MAX )
		, DispatchType( EVENT_DISPATCH_MAX )
		, FloatValue( 0.0f )
	{
	}
	...
	VRMenuEvent( eVRMenuEventType const eventType, eEventDispatchType const dispatchType,
		menuHandle_t const & targetHandle, menuHandle_t const & otherHandle, Vector3f const & floatValue, HitTestResult const & hitResult,
		char const * message )
		: EventType( eventType )
		, DispatchType( dispatchType )
		, TargetHandle( targetHandle )
