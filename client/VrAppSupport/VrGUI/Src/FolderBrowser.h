...
public:
	struct PanelView
	{
        ...
        PanelView( int id, GLuint textId )
            : Id( id )
            , TextureId( textId )
			, Visible( false )
            , MenuId( 0 )
        ...
        virtual menuHandle_t GetThumbnailHandle() { return Handle; }
		
		void LoadDefaultThumbnail( OvrGuiSys & guiSys, const GLuint defaultTextureId, const int thumbWidth, const int thumbHeight );
	...
	bool						GazingAtMenu( Matrix4f const & view ) const;
	...
	GLuint						GetDefaultThumbnailTextureId() const		{ return DefaultPanelTextureIds[ 0 ]; }
	void						QueueAsyncThumbnailLoad( const OvrMetaDatum * panoData, const int folderIndex, const int panelId );

protected:
	OvrFolderBrowser( OvrGuiSys & guiSys,
    ...
    virtual VRMenuComponent*    CreateFolderBrowserSwipeComponent( OvrFolderBrowser& folderBrowser, OvrFolderBrowser::FolderView * folder );
...
private:
	...
	void				BuildFolderView( OvrGuiSys & guiSys,
	...
	void				LoadFolderViewPanels( OvrGuiSys & guiSys,
										const OvrMetaData & metaData,
...
public:
	...
	OvrPanel_OnUp( VRMenu * menu, const OvrMetaDatum * panoData, int const folderIndex, int const panelId ) :
