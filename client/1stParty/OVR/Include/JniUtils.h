...
inline jclass ovr_GetLocalClassReferenceWithLoader( JNIEnv * jni, jobject classLoader, const char * className )
{
...
inline jclass ovr_GetGlobalClassReferenceWithLoader( JNIEnv * jni, jobject classLoader, const char * className )
{
...
inline jclass ovr_GetLocalClassReference( JNIEnv * jni, jobject activityObject, const char * className )
{
...
// This can be called from any thread but does need the activity object.
inline jclass ovr_GetGlobalClassReference( JNIEnv * jni, jobject activityObject, const char * className )
{
...
inline jmethodID ovr_GetMethodID( JNIEnv * jni, jclass jniclass, const char * name, const char * signature )
{
...
inline jmethodID ovr_GetStaticMethodID( JNIEnv * jni, jclass jniclass, const char * name, const char * signature )
{
...
inline const char * ovr_GetPackageCodePath( JNIEnv * jni, jobject activityObject, char * packageCodePath, int const maxLen )
{
...
inline bool ovr_GetInstalledPackagePath( JNIEnv * jni, jobject activityObject, char const * packageName,
		char * packagePath, int const packagePathSize )
{
	...
	jmethodID getPackageManagerMethod = jni->GetMethodID( activityClass, "getPackageManager", "()Landroid/content/pm/PackageManager;"  );
	...
	jmethodID getApplicationInfoMethod = jni->GetMethodID( PackageManagerClass, "getApplicationInfo", "(Ljava/lang/String;I)Landroid/content/pm/ApplicationInfo;" );
	...
	jint jZero = 0;
	jobject applicationInfo = jni->CallObjectMethod( packageManager, getApplicationInfoMethod, packageNameObj, jZero );
	if ( applicationInfo == NULL )
	{
		OVR_LOG( "ovr_GetInstalledPackagePath applicationInfo == NULL packageName='%s'", packageName );
		return false;
	}

	if ( jni->ExceptionOccurred() )
	{
		OVR_LOG( "ovr_GetInstalledPackagePath getApplicationInfo threw an exception: package not found." );
		jni->ExceptionClear();
		return false;
	}
	...
	jfieldID sourceDirField = jni->GetFieldID( ApplicationInfoClass, "sourceDir", "Ljava/lang/String;" );
...
inline const char * ovr_GetCurrentPackageName( JNIEnv * jni, jobject activityObject, char * packageName, int const maxLen )
{
	packageName[0] = '\0';
...
inline const char * ovr_GetCurrentActivityName( JNIEnv * jni, jobject activityObject, char * activityName, int const maxLen )
{
	activityName[0] = '\0';
...
inline jobject ovr_GetPackageSignatures( JNIEnv * jni, jobject activityObject, const char * packageName )
{
	...
	return jni->GetObjectField(
		packageInfo.GetJObject(),
		signaturesFieldId
		);
}
