...
def create_keystore(execfn, path, alias, storepw, keypw,
                    replace = False, sdn = None):
    """
    ...
    cmd = [ 'keytool', '-v', '-genkey', '-keyalg', 'RSA',
            '-keystore', path, '-storepass', storepw,
            '-alias', alias, '-keypass', keypw,
...
def genDebugKeystore():
    ...
    create_keystore(lambda x: ovrbuild.call(x),
        debug_props['keystore'], debug_props['alias'],
