...
OVR::ovrSurfaceDef OVRNode::CreateOVRSurface(size_t materialIndex, std::shared_ptr<Material> material)
{
	GlobalGraphicsResources& globalGraphicsResources = GlobalGraphicsResources::GetInstance();
	...
	{
		OVR_WARN("Mesh is null in CreateOVRSurface(...).");
		return ovr_surface_def; //Can't create surface without mesh.
	}
	...
	if(geo.indexCount==0)
	{
		OVR_WARN("indexCount is zero in CreateOVRSurface(...).");
		return ovr_surface_def; //Can't create surface without mesh.
	}
