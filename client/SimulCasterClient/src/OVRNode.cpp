...
OVR::GlProgram* OVRNode::GetEffectPass(const char* effectPassName)
{
...
OVR::ovrSurfaceDef OVRNode::CreateOVRSurface(size_t materialIndex, std::shared_ptr<scr::Material> material)
{
	GlobalGraphicsResources& globalGraphicsResources = GlobalGraphicsResources::GetInstance();
	OVR::GlProgram* effectPass = GetEffectPass(globalGraphicsResources.effectPassName);

	OVR::ovrSurfaceDef ovr_surface_def;
	...
	if(effectPassCreateInfo)
	{
		...
		ovr_surface_def.graphicsCommand.GpuState.blendEnable			= effectPassCreateInfo->colourBlendingState.blendEnable ? OVR::ovrGpuState::ovrBlendEnable::BLEND_ENABLE : OVR::ovrGpuState::ovrBlendEnable::BLEND_DISABLE;
