...
void main()
{
    ...
    vec4 initialLookup = textureLod(cubemapTexture, vSampleVec,0.0);
    float depth = initialLookup.a;
    ...
    vec3 colourSampleVec  = normalize(vec3(-pos_m.z, pos_m.x, pos_m.y));
    vec4 finalLookup=textureLod(cubemapTexture, colourSampleVec, 0.0);
//finalLookup.rgb+=fract(vEyeOffset);
    gl_FragColor = pow(finalLookup,vec4(.44,.44,.44,1.0));
